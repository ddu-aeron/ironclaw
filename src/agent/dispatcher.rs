...
mod tests {
    ...
    async fn test_need_approval_persists_first_auth_prompt_for_resume() {
        ...
        let result = agent
            .run_agentic_loop(&message, tenant, session, thread_id, initial_messages, CancellationToken::new())
            .await
    ...
    async fn test_dispatcher_terminates_with_all_tool_calls_failing() {
        ...
        let result = tokio::time::timeout(
            ...
            agent.run_agentic_loop(&message, tenant, session, thread_id, initial_messages, CancellationToken::new()),
        )
        .await;
